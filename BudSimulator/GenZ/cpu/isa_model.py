...
class ISASelector:
    ...
    def _select_isa_for_gemm(self, operator, data_type: str) -> Tuple[ISAType, float]:
        ...
        # Detect decode operations (M=1) and handle differently
        is_decode = (M == 1)
        
        if is_decode:
            # For decode (M=1), prefer smaller vector widths and better memory handling
            # AMX is inefficient for M=1, so avoid it
            
            # For decode, AVX2 often performs better than AVX512 due to:
            ...
            if ISAType.AVX512 in self.isa_configs and data_type in ['fp32', 'fp16', 'bf16']:
                if N * K > 4096:  # Large KV cache
                    ...
                    return ISAType.AVX512, 0.65  # Lower efficiency for small workloads
            ...
            return ISAType.SCALAR, 0.4  # Better than prefill scalar
        
        else:
            # Prefill/larger GEMM operations - original logic
    ...
    def _get_data_type_multiplier(self, data_type: str) -> float:
        """Get throughput multiplier for different data types"""
        # Relative to fp32 baseline
        multipliers = {
            'fp32': 1.0,
            'fp16': 2.0,    # ~2x faster than fp32
            'bf16': 2.0,    # ~2x faster than fp32  
            'int8': 4.0,    # ~4x faster than fp32
            'int32': 1.0,
        }
        return multipliers.get(data_type, 1.0)
