...
class HardwareRecommendation:
    ...
    def recommend_hardware(self, 
        ...
        - CPUs only recommended for small models (<14B params AND <40GB memory)
        ...
        Returns:
            Dict with:
            - cpu_recommendations: List of CPU hardware recommendations (empty for large models)
        ...
        for hw in all_hardware:
            ...
            # Only recommend CPUs for CPU-compatible workloads
            if hw['type'] == 'cpu':
                # Only add CPU recommendations for small models that are CPU compatible
                if cpu_compatible:
                    cpu_recommendations.append(hardware_rec)
                # For large models, skip CPU recommendations entirely
            else:
